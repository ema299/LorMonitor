...
| Export PDF sessione (base: snapshot + note) | **DONE 26/04** | Backend `backend/services/replay_pdf_service.py` (NEW) usa reportlab pure-Python (no system deps) — `render_replay_pdf(replay, notes_body, notes_updated_at)` ritorna bytes. Layout A4 minimal: title gold, subtitle metamonitor.app + ts UTC, sezione "Match metadata" tabella 8 righe (player/opp/perspective/winner/victory_reason/turn_count/game_id/uploaded), sezione "Session notes" con timestamp + body wrap, footer disclaimer. Endpoint `GET /api/v1/team/replay/{game_id}/export-pdf` in `backend/api/team.py` (`require_replay_owner`, attachment Content-Disposition, no-store cache). Frontend additivo: `tcExportPdf()` + button "Export PDF" gold-ghost nella toolbar notes (Board Lab) — additivo rispetto all'hardblock add-only su tab Team. Smoke: PDF generato 2426 bytes, magic `%PDF-1.4` OK. reportlab installato in venv. |
...
| Wiring cron/worker critici: import matches, KC batch, matchup reports, snapshot assembler, monitor unmapped, KC freshness, backup | **DONE 26/04** | 6 cron strumentati con try/except → `report_incident(source=..., severity='error', payload={error, traceback})` attorno al `__main__` block: `generate_killer_curves.py`, `refresh_kc_matchup_reports.py`, `monitor_unmapped_matches.py` (preserva SystemExit code), `monitor_kc_freshness.py` (idem), `import_kc_spy.py` (idem), `import_matches.py`. Pattern uniforme: lazy import del reporter + nested try/except (mai propaga errore del reporter). Smoke: `report_incident(source='wiring_smoke_test')` → row scritta correttamente in `ops_incidents`, payload OK, cleanup OK. Backup script bash gestito a livello cron (skip — non Python). Snapshot assembler chiamato in-process da FastAPI, errori già catturati da `global_exception_handler`. |
| `daily_health_digest.py` 07:30 UTC → mail unica a `monitorteamfe@gmail.com` | **DONE 26/04** | `scripts/daily_health_digest.py` (NEW): legge ultime 24h `user_feedback status=new` + `ops_incidents status=new severity>=warn`, formatta digest text con header window/counts + sezioni FEEDBACK by kind + INCIDENTS by severity. SMTP via Gmail (riusa pattern `cron_health_report.py`), `LM_SMTP_DISABLED=1` per dry-run. Su send success marca rows come `digested`. Smoke dry-run OK. **Cron schedule attivo**: `30 7 * * * cd /mnt/HC_Volume_104764377/finanza/Lor/Analisi_deck/App_tool && venv/bin/python3 scripts/daily_health_digest.py >> /var/log/lorcana-health-digest.log 2>&1` aggiunto al crontab root il 26/04 (backup `/tmp/crontab_backup_1777225677.bak`). |
