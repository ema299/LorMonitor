...
| **Backend: send password reset email** — `backend/api/auth.py:134` ha `# TODO: send email with reset link`. Wirare con SMTP esistente (`/tmp/.smtp_pass`, sender `monitorteamfe@gmail.com`). Subject "Reset your Lorcana Monitor password", body con link `https://metamonitor.app/dashboard.html?reset_token=<token>`. Senza questo, il flusso forgot password frontend è inutile (token generato ma non recapitato). | 1h | **P0 prima del go-live** |
