# VENDORED da legacy loader.py @ commit 58288f36a6e41b1830efab0941223e7160b84450
...
LEGALITY_SKIP_PATH = os.path.join(MATCHES_DIR, ".legality_skip.json")
...
# Module-level legality skip list (lazy-loaded once)
_legality_skip = None

def _load_legality_skip():
    """Load legality skip list (set of file paths to exclude). Cached after first load."""
    global _legality_skip
    if _legality_skip is not None:
        return _legality_skip
    _legality_skip = set()
    if os.path.exists(LEGALITY_SKIP_PATH):
        try:
            data = json.load(open(LEGALITY_SKIP_PATH))
            _legality_skip = set(data.get("skip", []))
        except Exception:
            pass
    return _legality_skip
...
def load_matches(our_deck, opp_deck, max_turn=None, game_format='core'):
    ...
    for date_dir in sorted(os.listdir(MATCHES_DIR)):
        ...
        for root, dirs, files in os.walk(date_path):
            ...
            skip_set = _load_legality_skip()
            for f in files:
                if not f.endswith('.json'):
                    continue
                fpath = os.path.join(root, f)

                # Legality gate: skip matches flagged by audit_legality.py
                if skip_set and fpath in skip_set:
                    continue
...
def build_extended_pool(deck_code, cards_db):
    ...
    for name, d in cards_db.items():
        ink = d.get('ink', '').lower()
        # Dual ink: "amethyst/sapphire" — playable ONLY in decks with BOTH colors
        ink_match = (ink in colors or ink == 'dual ink'
                     or ('/' in ink and all(c.strip() in colors for c in ink.split('/'))))
        if not ink_match:
            continue
