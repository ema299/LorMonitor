...
class TeamRoster(Base):
    """Roster — legacy team-wide rows (coach_id NULL) + B.7.2 coach-managed
    students. ENUM-as-VARCHAR pattern (matches users.tier).

    Migration ``d5a8f3e1c2b9`` added the Coach Workspace columns.
    """

    __tablename__ = "team_roster"

    id = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    name = mapped_column(String(100), nullable=False)  # legacy, kept for backward compat
    role = mapped_column(String(50), nullable=True)
    added_at = mapped_column(DateTime, server_default=func.now())

    # B.7.2 Coach Workspace columns
    coach_id = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="CASCADE"),
        nullable=True,
    )
    student_user_id = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("users.id", ondelete="SET NULL"),
        nullable=True,
    )
    display_name = mapped_column(String(100), nullable=True)
    status_admin = mapped_column(String(20), nullable=False, server_default="active")
    duels_nick = mapped_column(String(100), nullable=True)
    duels_nick_status = mapped_column(String(20), nullable=False, server_default="missing")
    discord_username = mapped_column(String(50), nullable=True)
    discord_id = mapped_column(String(30), nullable=True)
